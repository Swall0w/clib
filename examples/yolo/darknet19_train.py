...
def arg():
    ...
    parser.add_argument('--weight_decay', '-wd', default=0.0005,
                        help='WeightDecay')
    parser.add_argument('--resize', '-s', type=int, default=224,
                        help='number of unit')
...
def main():
    ...
    updater = training.StandardUpdater(
        train_itr, optimizer, converter=darknet_converter, device=args.gpu)
    ...
    trainer.extend(extensions.PrintReport(
        ['epoch', 'iteration', 'main/loss', 'main/accuracy', 'lr']))
    trainer.extend(extensions.ProgressBar())

    trainer.run()
    if args.resize == 448:
        chainer.serializers.save_npz(
            args.output+'/darknet19_448.weights', model.to_cpu())
    else:
        chainer.serializers.save_npz(
            args.output+'/darknet19.weights', model.to_cpu())
    print('end')
